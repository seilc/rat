...
static void Tridiag_Solve(F32* a, F32* b, F32* c, xVec3* d, xVec3* x, S32 n)
{
    ...
    gamma = (F32*)RwMallocL(n * sizeof(F32), rwMEMHINTDUR_NADURATION, 106);
    delta = (xVec3*)RwMallocL(n * sizeof(xVec3), rwMEMHINTDUR_NADURATION, 107);
...
static void Interpolate_Bspline(xVec3* data, xVec3* control, F32* knots, U32 nodata)
{
    U32 i;
    F32* alpha, *beta, *gamma;
    F32 t1, t2, t3, t4, t5;
    
    alpha = (F32*)RwMallocL(nodata * sizeof(F32), rwMEMHINTDUR_NADURATION, 214);
    beta = (F32*)RwMallocL(nodata * sizeof(F32), rwMEMHINTDUR_NADURATION, 215);
    gamma = (F32*)RwMallocL(nodata * sizeof(F32), rwMEMHINTDUR_NADURATION, 216);
...
static F32 ArcLength3(xCoef3* coef, F64 ustart, F64 uend)
{
    U32 i;
    F64 A, B, C, D, E, h, sum, u;

    A = 9.0 * ((F64)coef->x.a[0] * (F64)coef->x.a[0] + (F64)coef->y.a[0] * (F64)coef->y.a[0] + (F64)coef->z.a[0] * (F64)coef->z.a[0]);
    B = 12.0 * ((F64)coef->x.a[0] * (F64)coef->x.a[1] + (F64)coef->y.a[0] * (F64)coef->y.a[1] + (F64)coef->z.a[0] * (F64)coef->z.a[1]);
    C = 6.0 * ((F64)coef->x.a[0] * (F64)coef->x.a[2] + (F64)coef->y.a[0] * (F64)coef->y.a[2] + (F64)coef->z.a[0] * (F64)coef->z.a[2]) +
        4.0 * ((F64)coef->x.a[1] * (F64)coef->x.a[1] + (F64)coef->y.a[1] * (F64)coef->y.a[1] + (F64)coef->z.a[1] * (F64)coef->z.a[1]);
    D = 4.0 * ((F64)coef->x.a[1] * (F64)coef->x.a[2] + (F64)coef->y.a[1] * (F64)coef->y.a[2] + (F64)coef->z.a[1] * (F64)coef->z.a[2]);
    E = (F64)coef->x.a[2] * (F64)coef->x.a[2] + (F64)coef->y.a[2] * (F64)coef->y.a[2] + (F64)coef->z.a[2] * (F64)coef->z.a[2];
    h = (uend - ustart) / 50.0;
    sum = 0.0;
    u = ustart + h;
...
// ???
    ...
    return (sum +
        sqrt((((A * ustart + B) * ustart + C) * ustart + D) * ustart + E) +
        sqrt((((A * uend + B) * uend + C) * uend + D) * uend + E))
        * h / 3.0;
}

static void EvalCoef3(xCoef3* coef, F32 u, U32 deriv, xVec3* o)
{
    switch (deriv) {
    case 0:
...
static void BasisToCoef3(xCoef3* coef, F32 N[4][4], xVec3* p0, xVec3* p1, xVec3* p2, xVec3* p3)
{
    U32 c;
    for (c = 0; c < 4; c++) {
...
static void CoefToUnity3(xCoef3* dest, xCoef3* coef, F32 t1, F32 t2)
{
    U32 i;
    F32 a, b, c, d;
    F32* p, *o;
...
static void BasisBspline(F32 N[7][4], F32* t)
{
    U32 i, k, c;
    F32 d1, d2;
    
    N[0][3] = 0.0f, N[1][3] = 0.0f, N[2][3] = 0.0f, N[3][3] = 1.0f,
        N[4][3] = 0.0f, N[5][3] = 0.0f, N[6][3] = 0.0f;
    ...
    for (k = 2; k <= 4; k++) {
        for (i = 0; i < 8 - k; i++) {
            ...
            for (c = 5 - k; c <= 3; c++) {
                ...
                Ntemp[c] += -t[i] * N[i][c] * d1 + t[i + k] * N[i + 1][c] * d2;
            }
...
static U32 SegBspline(xSpline3* spl, F32 u)
{
    U32 min, max, mid;
...
static F32 ArcEvalIterate(xSpline3* spl, F32 s, U32 deriv, xVec3* o, U32 iterations)
{
    xCoef3* coef, tempCoef;
...
void xSpline3_ArcInit(xSpline3* spl, U32 sample)
{
    ...
    if (oldalloc < spl->N * sample) {
        spl->arcLength = (F32*)xMEMALLOC(spl->arcSample * spl->allocN * sizeof(F32), 0, eMemMgrTag_Spline, 0, 775);
    }
...
static xSpline3* AllocSpline3(xVec3* points, F32* time, U32 numpoints, U32 numalloc, U32 flags, U32 type)
{
    ...
    spl = (xSpline3*)xMEMALLOC(sizeof(xSpline3), 0, eMemMgrTag_Spline, 0, 904);
    ...
    spl->points = (xVec3*)xMEMALLOC((spl->allocN + 1) * sizeof(xVec3), 0, eMemMgrTag_Spline, 0, 920);
    memcpy(spl->points, points, (spl->N + 1) * sizeof(xVec3));

    if (time) {
        spl->time = (F32*)xMEMALLOC((spl->allocN + 1) * sizeof(F32), 0, eMemMgrTag_Spline, 0, 925);
...
xSpline3* xSpline3_Bezier(xVec3* points, F32* time, U32 numpoints, U32 numalloc, xVec3* p1, xVec3* p2)
{
    ...
    spl = AllocSpline3(points, time, numpoints, numalloc, 0, 3);
    spl->p12 = (xVec3*)xMEMALLOC(spl->allocN * 2 * sizeof(xVec3), 0, eMemMgrTag_Spline, 0, 1013);
...
DECOMP_FORCEACTIVE(
    ...
    "%s",
    "spl->arcLength",
    "Bspline --> Coef error:     %f\n",
    "Bspline --> Bezier error:   %f\n",
    "Bspline --> Hermite error:  %f\n",
    "Arclength 1,1,1 check -->  %f == %f\n",
    "Arc[%f]== %f %f %f %f\n"
)
